...
Еще одна особенность: запросы с показателем *спрос* (DEMAND) равным нулю не собираются в БД несмотря на то, что в статистике ЯВМ и API они отражены. Это необходимо для экономии места. На арифметические операции с данными это повлиять не должно, т.к. не "не было ввода запроса - не было и статистики".
